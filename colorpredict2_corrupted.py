...
def build_safety_graph(grid_df):
    ...
    safety_scores = dict(zip(nodes, grid_df['unsafe_prob'].values))
    
    for i, node in enumerate(nodes):
        neighbors = []
        safety_score = grid_df.iloc[i]['unsafe_prob']
        
        # Connect to nearby nodes within reasonable distance
        for j, other_node in enumerate(nodes):
            if i != j:
                dist = heuristic(node, other_node)
                # Connect nearby nodes (within 0.01 degrees ~ 1 km)
                if 0 < dist < 0.01:
                    neighbors.append((other_node, dist, safety_scores[other_node]))
...
<body>
    ...
    <script>
        ...
        async function getRouteFromOSRM(startLat, startLon, endLat, endLon) {{
            const url = `https://router.project-osrm.org/route/v1/driving/${{startLon}},${{startLat}};${{endLon}},${{endLat}}?overview=full&geometries=geojson&annotations=duration,distance,speed`;
            try {{
                const response = await fetch(url);
                const data = await response.json();
        ...
        findRouteBtn.addEventListener('click', async () => {{
            ...
            try {{
                // Use A* for combined safety + distance optimization
                ...
                if (!response.ok) throw new Error('A* backend not available');
                
                const astarRoute = await response.json();
                
                if (astarRoute.path && astarRoute.path.length > 0) {{
                    // Get OSRM route for traffic info
                    const osrmRoute = await getRouteFromOSRM(startLat, startLon, endLat, endLon);
                    
                    // Draw A* optimized route in blue with traffic colors
                    await drawRouteWithTomTomTraffic(astarRoute.path);
                    
                    statusDiv.className = 'status success';
                    statusDiv.innerHTML = 'Optimal route found (Safest & Shortest)';
                    
                    const safetyScore = (1 - astarRoute.average_safety).toFixed(2);
                    const safetyLevel = astarRoute.average_safety < 0.3 ? 'Safe' : astarRoute.average_safety < 0.7 ? 'Moderate' : 'Unsafe';
                    
                    routeInfoDiv.style.display = 'block';
                    routeInfoDiv.innerHTML = `
                        <strong>Optimized Route:</strong><br>
                        Waypoints: ${{astarRoute.path.length}}<br>
                        Safety Score: ${{safetyScore}}<br>
                        Safety Level: ${{safetyLevel}}<br>
                        <small style="color: #666;">Optimizes both safety & distance</small>
                    `;
                    ...
                    if (osrmRoute) {{
                        ...
                        trafficInfoDiv.innerHTML = `
                            ...
                            Traffic Delay: +${{osrmRoute.trafficDelay/60}} min<br>
                            Current Traffic: ${{getTrafficInfo()}}
                        `;
                    }}
                    
                    map.fitBounds(L.polyline(astarRoute.path).getBounds());
                }} else {{
                    throw new Error('No path found');
                }}
            }} catch (error) {{
                console.log('A* optimization failed, using OSRM fallback');
...
if __name__ == "__main__":
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .sidebar h1 {{ color: #2c3e50; font-size: 20px; margin-bottom: 20px; }}
...
<body>
    <div class="container">
        <div class="sidebar">
            <h1>🛡️ Safety Route Finder</h1>
            
            <button id="liveLocationBtn" style="background: #3498db; margin-bottom: 10px;">📍 Start Live Location</button>
    ...
    <script>
        ...
        gradientData.forEach(point => {{
            ...
            L.circleMarker([point.lat, point.lon], {{
                ...
                fillOpacity: opacity,
                weight: 1,
                popup: 'Risk: ' + (point.risk * 100).toFixed(1) + '%',
                tooltip: 'Unsafe Probability: ' + (point.risk * 100).toFixed(1) + '%'
            }}).addTo(map);
        }});
        ...
        function getTrafficFactor() {{
            const hour = new Date().getHours();
            if (hour >= 7 && hour <= 10) return 1.5; // Morning rush (50% delay)
            if (hour >= 17 && hour <= 20) return 1.8; // Evening rush (80% delay)
            if (hour >= 11 && hour <= 16) return 1.2; // Midday (20% delay)
            if (hour >= 21 || hour <= 6) return 1.05; // Night (5% delay)
            return 1.0;
        }}
        ...
        // Geocoding function using Nominatim (Open Source)
        async function geocode(query) {{
            const url = `https://nominatim.openstreetmap.org/search?format=json&q=${{query}},Andheri,Mumbai,India&limit=5`;
            try {{
                const response = await fetch(url);
        ...
        // Get route from OSRM (Open Source)
        async function getRouteFromOSRM(startLat, startLon, endLat, endLon) {{
            const url = `https://router.project-osrm.org/route/v1/driving/${{startLon}},${{startLat}};${{endLon}},${{endLat}}?overview=full&geometries=geojson&annotations=duration,distance,speed`;
            try {{
                const response = await fetch(url);
        ...
        liveLocationBtn.addEventListener('click', () => {{
            if (!isLiveLocation) {{
                if (navigator.geolocation) {{
                    ...
                    watchId = navigator.geolocation.watchPosition(
                        position => {{
                            ...
                            isLiveLocation = true;
                            liveLocationBtn.textContent = '⏹️ Stop Live Location';
                            liveLocationBtn.style.background = '#e74c3c';
                ...
                liveMarker = null;
                isLiveLocation = false;
                liveLocationBtn.textContent = '📍 Start Live Location';
                liveLocationBtn.style.background = '#3498db';
                locationInfoDiv.style.display = 'none';
                statusDiv.textContent = 'Live location stopped.';
            }}
        ...
        findRouteBtn.addEventListener('click', async () => {{
            ...
            // Find route
            statusDiv.className = 'status loading';
            statusDiv.textContent = '🔄 Computing safest route with real-time traffic...';
            routeInfoDiv.style.display = 'none';
            
            const routeData = await getRouteFromOSRM(startLat, startLon, endLat, endLon);
            
            if (routeData) {{
                // Draw route with TomTom real-time traffic
                await drawRouteWithTomTomTraffic(routeData.coordinates);
                
                statusDiv.className = 'status success';
                statusDiv.innerHTML = 'Route found with real-time traffic!<br>Check details below';
                
                routeInfoDiv.style.display = 'block';
                routeInfoDiv.innerHTML = `
                    <strong>📊 Route Details:</strong><br>
                    📏 Distance: ${{routeData.distance}} km<br>
                    ⏱️ Base Duration: ${{routeData.duration}} min<br>
                    🚦 Traffic: ${{routeData.traffic}}<br>
                    🚗 Adjusted Duration: ${{routeData.adjustedDuration}} min<br>
                    ⚠️ Traffic Delay: +${{routeData.trafficDelay}} min<br>
                    <br>
                    <strong>Route Color: Blue (Dashed Line)</strong>
                `;
